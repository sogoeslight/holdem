...
class RoyalFlushCheckerTest {

    private final RankChecker subject = new RoyalFlushChecker();

    public static Stream<Arguments> testData() {
        return Stream.of(
                Arguments.of(createSevenCards("AsKsQsJsTs", "4d4s"), createCardList("AsKsQsJsTs")),
                Arguments.of(createSevenCards("AcTcKcJcQc", "Td3s"), createCardList("AcKcQcJcTc")),
                Arguments.of(createSevenCards("AsKsQs2s5s", "JsTs"), createCardList("AsKsQsJsTs"))
        );
    }
    ...
    void check_happy_path(SevenCards sevenCards, List<Card> result) {
        ...
        assertThat(actual.get().getRank()).isEqualTo(CombinationRank.ROYAL_FLUSH);
    }
